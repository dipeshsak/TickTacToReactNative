...
class App extends Component {
  ...
  winGame=()=>{
    ...
    //column
      ...
      this.setState({
        winMessage :(itemArray[1] ? 'Cross' : 'Circle').concat('Won')
      })
    }
    else if((itemArray[2] !== 'empty')&& (itemArray[2] === itemArray[5]) && (itemArray[5] === itemArray[8])){
      this.setState({
        winMessage :(itemArray[2] ? 'Cross' : 'Circle').concat(' Won')
      })
    }
    //diagonal
  ...
  render() {
    ...
    <Button warning onPress={()=>this.resetGame()}><Text>Reset Game</Text></Button>
    </View>
    )
  }
}
